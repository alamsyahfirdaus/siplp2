  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="" method="post" id="form">
        <input type="text" name="id_program_studi" value="" style="display: none;">
        <div class="modal-body">
          <div class="form-group">
            <label for="program_studi">Program Studi<span style="color: #dc3545; font-weight: bold;">*</span></label>
            <input type="text" class="form-control" id="program_studi" name="program_studi" placeholder="Program Studi" autocomplete="off">
            <span id="error-program_studi" class="error invalid-feedback" style="display: none;"></span>
          </div>
        </div>
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    var url_table = 'master/show_department/<?= md5(time()) ?>';
  ...
  function add_edit(id) {
    ...
    if (id) {
      var program_studi = $('#program_studi_'+ id +'').text();

      $('.modal-title').text('Edit ' + title);
      $('[name="id_program_studi"]').val(id).change();
      $('[name="program_studi"]').val(program_studi).change();
    } else {
  ...
  function save_data() {
    $.ajax({
        url: '<?= site_url('master/save_department/' . md5(time())) ?>',
        type: 'POST',
